...
struct Tcp4Socket {
	...
	bind(void *object, const char *creds, const void *addrPtr, size_t addrLength) {
		...
		TcpEndpoint bindEp;
		if(auto e = checkAddress(addrPtr, addrLength, bindEp);
	...
	connect(void *object, const char *creds, const void *addrPtr, size_t addrLength) {
		...
		if(auto e = checkAddress(addrPtr, addrLength, connectEp);
		   e != protocols::fs::Error::none) {
			co_return e;
		}
	...
	static async::result<void> setFileFlags(void *object, int flags) {
		auto self = static_cast<Tcp4Socket *>(object);
		std::cout << "posix: setFileFlags on tcp socket only supports O_NONBLOCK"
			  << std::endl;
		if(flags & ~O_NONBLOCK) {
			std::cout << "posix: setFileFlags on tcp socket called with unknown flags"
				  << std::endl;
			co_return;
		}
		if(flags & O_NONBLOCK) {
