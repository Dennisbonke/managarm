...
using AnyFilter = std::variant<NoFilter, EqualsFilter, Conjunction>;
...
struct Conjunction {
	Conjunction(std::vector<AnyFilter> operands) : _operands(std::move(operands)) {}
...
using AnyItem = std::variant<StringItem, ListItem>;
...
struct Entity {
	...
	async::result<Entity>
	createObject(std::string name, const Properties &properties, ObjectHandler handler) const;

	// links an observer to this group.
	async::result<Observer>
	linkObserver(const AnyFilter &filter, ObserverHandler handler) const;
...
struct AttachEvent {
	...
	const Properties &getProperties() { return _properties; }
