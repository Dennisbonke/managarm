...
constexpr static uint8_t getStatus = 0;
constexpr static uint8_t clearFeature = 1;
constexpr static uint8_t setFeature = 3;
constexpr static uint8_t getDescriptor = 6;
}  // namespace ClassRequests

namespace PortBits {
constexpr static uint16_t connect = 0x01;
constexpr static uint16_t enable = 0x02;
constexpr static uint16_t reset = 0x10;
constexpr static uint16_t lowSpeed = 0x200;
constexpr static uint16_t highSpeed = 0x400;
}  // namespace PortBits

namespace PortFeatures {
// static constexpr uint16_t connect = 0;
// static constexpr uint16_t enable = 1;
constexpr static uint16_t reset = 4;
constexpr static uint16_t connectChange = 16;
constexpr static uint16_t enableChange = 17;
constexpr static uint16_t resetChange = 20;
}  // namespace PortFeatures
	...
	while(true) {
		...
		for(size_t port = 0; port < state_.size(); port++) {
			...
			(co_await device_.transfer(
				 ControlTransfer {kXferToHost, statusReq, result.view_buffer()}
			 ))
				.unwrap();
			...
			if(result[1] & PortBits::connect) {
				...
				(co_await device_.transfer(ControlTransfer {
					 kXferToDevice,
					 clearReq,
...
async::result<frg::expected<UsbError, DeviceSpeed>> StandardHub::issueReset(int port) {
	...
	FRG_CO_TRY(co_await device_.transfer(
		ControlTransfer {kXferToDevice, resetReq, arch::dma_buffer_view {}}
	));
