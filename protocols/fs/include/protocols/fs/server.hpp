...
struct FileOperations {
	...
	constexpr FileOperations &withRead(async::result<ReadResult> (*f)(
		void *object,
		const char *,
		void *buffer,
		size_t length
	...
	constexpr FileOperations &withTruncate(
		async::result<frg::expected<protocols::fs::Error>> (*f)(void *object, size_t size)
	) {
		truncate = f;
		return *this;
	}
	...
	constexpr FileOperations &
	withFallocate(async::result<frg::expected<protocols::fs::Error>> (*f)(
		void *object,
		int64_t offset,
		size_t size
	)) {
	...
	constexpr FileOperations &withSockname(
		async::result<size_t> (*f)(void *object, void *addr_ptr, size_t max_addr_length)
	) {
		sockname = f;
		return *this;
	}
	...
	async::result<frg::expected<Error, PollStatusResult>> (*pollStatus)(void *object);
