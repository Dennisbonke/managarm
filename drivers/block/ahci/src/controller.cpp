	...
	if(version >= 0x10200 && (cap2 & flags::cap2::supportsHandoff)) {
		...
		if(success) {
			...
			if(regs_.load(regs::biosHandoff) & flags::bohc::biosBusy) {
				...
				std::cout
					<< "block/ahci: BIOS handoff timed out once, assuming control\n";
			}
		}
	}
...
async::detached Controller::handleIrqs_() {
	...
	while(true) {
		...
		if(intStatus) {
			...
			if(useMsis_) {
				// In theory, we shouldn't get here, but refer to xhci handleMsis()
				// comment for details.
				HEL_CHECK(helAcknowledgeIrq(
					irq_.getHandle(),
					kHelAckAcknowledge,
				...
				for(auto &port : activePorts_) {
					std::cout << "block/ahci: NACK, dumping state on port "
						  << port->getIndex() << ":\n";
					port->dumpState();
					port->checkErrors();
					std::cout
						...
						<< regs_.load(regs::interruptStatus)
						<< ", masked GIS "
						<< (regs_.load(regs::interruptStatus) & portsImpl_)
						<< ", read GIS " << intStatus << "\n";
				}
