...
async::result<bool> Port::init() {
	...
	if(staggeredSpinUp_) {
		...
		// Wait up to 10ms for PxSSTS.DET = 1 or 3 (AHCI spec: 10.1.1, SATA 3.2
		// spec: 17.7.2)
...
void Port::dumpState() {
	...
	printf("  submittedCmds slots used: %zu\n",
	       std::count_if(submittedCmds_.begin(), submittedCmds_.end(), [](auto &p) {
		       return p != nullptr;
	       }));
}
...
void Port::handleIrq() {
	...
	// TODO: If we have a lot of waiters, this will cause many spurious wakeups. Ideally, we
	// only notify a certain number of tasks, and the rest can stay asleep.
