...
async::result<void> enumerateCtl() {
	...
	auto filter = mbus::Conjunction({mbus::EqualsFilter("class", "kernletctl")});
...
upload(const void *elf, size_t size, std::vector<BindType> bind_types) {
	...
	for(auto bt : bind_types) {
		...
		case BindType::offset:
			proto = managarm::kernlet::OFFSET;
			break;
		case BindType::memoryView:
			proto = managarm::kernlet::MEMORY_VIEW;
			break;
		case BindType::bitsetEvent:
			proto = managarm::kernlet::BITSET_EVENT;
			break;
		default:
	...
	auto [offer, send_req, send_data, recv_resp, pull_kernlet] =
...
async::detached serveCompiler(helix::UniqueLane lane) {
	while(true) {
		auto [accept, recv_req, recv_code] = co_await helix_ng::exchangeMsgs(
			lane,
			helix_ng::accept(helix_ng::recvInline(), helix_ng::recvInline())
		);
		...
		if(req.req_type() == managarm::kernlet::CntReqType::COMPILE) {
			...
			for(int i = 0; i < req.bind_types_size(); i++) {
				...
				BindType bt;
				switch(proto) {
				case managarm::kernlet::OFFSET:
					bt = BindType::offset;
					break;
				case managarm::kernlet::MEMORY_VIEW:
					bt = BindType::memoryView;
					break;
				case managarm::kernlet::BITSET_EVENT:
			...
			auto elf = compileFafnir(
				reinterpret_cast<const uint8_t *>(recv_code.data()),
				recv_code.length(),
				bind_types
			);
			recv_code.reset();
