...
constexpr inline bool ignoreControllerSpeeds = true;
...
inline frg::expected<UsbError> completionToError(Event ev) {
	...
	case 1:
		return frg::success;
	case 13:
		return frg::success;  // Should short packets always be treated as success?
	case 3:
...
async::detached Controller::initialize() {
	...
	auto usb_legacy_cap =
		std::find_if(caps.begin(), caps.end(), [](auto &a) { return a.first == 0x1; });
	...
	for(auto &p : _supportedProtocols) {
		...
		constexpr const char *exponent[] = {"B/s", "Kb/s", "Mb/s", "Gb/s"};

		constexpr const char *type[] =
			{"Symmetric", "Reserved", "Asymmetric Rx", "Asymmetric Tx"};

		constexpr const char *linkProtocol[] =
			{"SuperSpeed", "SuperSpeedPlus", "Reserved", "Reserved"};

		printf("xhci: supported speeds:\n");
		for(auto &s : p.speeds) {
			printf("xhci:\tspeed:%u %s\n", s.mantissa, exponent[s.exponent]);
	...
	_scratchpadBufArray =
		arch::dma_array<uint64_t> {&_memoryPool, static_cast<size_t>(max_scratchpad_bufs)};
	for(size_t i = 0; i < max_scratchpad_bufs; i++) {
		_scratchpadBufs.push_back(arch::dma_buffer(&_memoryPool, page_size));
	...
	_operational.store(
		op_regs::dcbaap,
		helix::ptrToPhysical(_dcbaa.data())
	);  // tell the device about our dcbaa
	...
	_operational.store(
		op_regs::usbcmd,
		usbcmd::run(1) | usbcmd::intrEnable(1)
	);  // enable interrupts and start hcd
...
async::result<void>
Controller::enumerateDevice(std::shared_ptr<Hub> parentHub, int port, DeviceSpeed speed) {
	uint32_t route = 0;
	...
	(co_await device
		 ->enumerate(rootPort, port, route, parentHub, speed, proto->protocolSlotType))
		.unwrap();
	...
	if(descriptor->deviceClass == 0x09 && descriptor->deviceSubclass == 0) {
		auto hub =
			(co_await createHubFromDevice(parentHub, ::Device {device}, port)).unwrap();
...
async::detached Controller::Port::initPort() {
	...
	if(_proto->major == 2) {
		...
		if(linkStatus != 7) {
			printf("\e[35m"
			       "xhci: USB2 port did not enter polling state after CCS=1"
			       "\e[39m\n");
		}
	...
	if(ignoreControllerSpeeds || _proto->speeds.empty()) {
		...
		for(auto &pSpeed : _proto->speeds) {
			if(pSpeed.value == speedId) {
				if(pSpeed.exponent == 2 && pSpeed.mantissa == 12) {
					// Full Speed
					speed = DeviceSpeed::fullSpeed;
				} else if(pSpeed.exponent == 1 && pSpeed.mantissa == 1500) {
					// Low Speed
					speed = DeviceSpeed::lowSpeed;
				} else if(pSpeed.exponent == 2 && pSpeed.mantissa == 480) {
					// High Speed
					speed = DeviceSpeed::highSpeed;
				} else if(pSpeed.exponent == 3 && (pSpeed.mantissa == 5 || pSpeed.mantissa == 10 || pSpeed.mantissa == 20)) {
					// SuperSpeed
					speed = DeviceSpeed::superSpeed;
				} else if(pSpeed.exponent == 2 && (pSpeed.mantissa == 1248 || pSpeed.mantissa == 2496 || pSpeed.mantissa == 4992 || pSpeed.mantissa == 1458 || pSpeed.mantissa == 2915 || pSpeed.mantissa == 5830)) {
					// SSIC SuperSpeed
					speed = DeviceSpeed::superSpeed;
				}
...
, _proto {&proto} {
	for(size_t i = 0; i < proto.compatiblePortCount; i++) {
		_ports.push_back(
			std::make_unique<Port>(i + proto.compatiblePortStart, controller, &proto)
		);
...
Controller::Device::Device(Controller *controller) : _slotId {-1}, _controller {controller} {}
...
async::result<frg::expected<UsbError, std::string>> Controller::Device::configurationDescriptor() {
...
Controller::Device::useConfiguration(int number) {
	...
	Transfer::buildControlChain(
		[&](RawTrb trb, bool last) {
			if(last) {
				trb.val[3] |= 1 << 5;  // IOC
			}
			pushRawTransfer(0, trb, last ? &comp : nullptr);
		},
		{.request = request_type::setConfig,
	...
	co_return Configuration {std::make_shared<Controller::ConfigurationState>(
		_controller,
		shared_from_this(),
		number
	)};
...
async::result<frg::expected<UsbError>> Controller::Device::transfer(ControlTransfer info) {
...
inline static uint8_t getHcdSpeedId(DeviceSpeed speed) {
	switch(speed) {
...
async::result<frg::expected<UsbError>> Controller::Device::enumerate(
	size_t rootPort,
	size_t port,
	uint32_t route,
	...
	if((speed == DeviceSpeed::lowSpeed || speed == DeviceSpeed::fullSpeed) && hub->parent()) {
...
Controller::Device::readDescriptor(arch::dma_buffer_view dest, uint16_t desc) {
	...
	Transfer::buildControlChain(
		...
		{.type = 0x80,
		 .request = request_type::getDescriptor,
		 .value = static_cast<uint16_t>(desc),
		 .length = static_cast<uint16_t>(dest.size())},
		dest,
		true
	);
...
inline static uint32_t getHcdEndpointType(PipeType dir, EndpointType type) {
...
inline static uint32_t getDefaultAverageTrbLen(EndpointType type) {
	if(type == EndpointType::control) {
		return 8;
	}
	if(type == EndpointType::isochronous) {
...
async::result<frg::expected<UsbError>> Controller::Device::setupEndpoint(
	int endpoint,
	PipeType dir,
...
void Controller::Device::_initEpCtx(
	InputContext &ctx,
	int endpoint,
	PipeType dir,
	size_t maxPacketSize,
	...
	int endpointId = endpoint * 2 + ((dir == PipeType::in || dir == PipeType::control) ? 1 : 0);
...
Controller::ConfigurationState::ConfigurationState(
	Controller *controller,
...
Controller::ConfigurationState::useInterface(int number, int alternative) {
	assert(!alternative);
	co_return Interface {
		std::make_shared<Controller::InterfaceState>(_controller, _device, number)};
}
...
Controller::InterfaceState::InterfaceState(
	Controller *controller,
	std::shared_ptr<Device> device,
	int
)
...
Controller::InterfaceState::getEndpoint(PipeType type, int number) {
	co_return Endpoint {
		std::make_shared<Controller::EndpointState>(_controller, _device, number, type)};
}
...
Controller::EndpointState::EndpointState(
	Controller *,
	std::shared_ptr<Device> device,
	int endpoint,
	PipeType type
...
async::result<frg::expected<UsbError>> Controller::EndpointState::transfer(ControlTransfer info) {
...
async::result<frg::expected<UsbError, size_t>> Controller::EndpointState::transfer(BulkTransfer info
) {
	int endpointId = _endpoint * 2 + (_type == PipeType::in ? 1 : 0);
