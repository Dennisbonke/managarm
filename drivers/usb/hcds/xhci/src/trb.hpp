...
constexpr RawTrb evaluateContext(uint8_t slotId, uintptr_t inputCtx) {
	...
	return RawTrb {
		...
		0,
		(uint32_t {slotId} << 24)
			| (static_cast<uint32_t>(TrbType::evalContextCommand) << 10)};
}
}  // namespace Command
...
constexpr RawTrb setupStage(SetupPacket setup, bool hasDataStage, bool dataIn) {
	return RawTrb {
		(uint32_t {setup.value} << 16) | (uint32_t {setup.request} << 8)
			| uint32_t {setup.type},
...
constexpr RawTrb statusStage(bool dataIn) {
	return RawTrb {
		...
		0,
		0,
		((dataIn ? 1 : 0) << 16) | (static_cast<uint32_t>(TrbType::statusStage) << 10)};
}
...
inline void
buildTransferChain(FU use, bool specifyFinal, arch::dma_buffer_view view, FB build, Ts... ts) {
	size_t progress = 0;
