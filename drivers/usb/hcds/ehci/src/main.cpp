...
Enumerator::Enumerator(Controller *controller) : _controller {controller} {}
...
void Controller::_checkPorts() {
	...
	for(int i = 0; i < _numPorts; i++) {
		...
		//		std::cout << "port " << i << " sc: " << static_cast<unsigned int>(sc)
		//<< std::endl;
...
async::result<frg::expected<UsbError>>
Controller::_directTransfer(ControlTransfer info, QueueEntity *queue, size_t max_packet_size) {
