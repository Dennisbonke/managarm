...
Inode::link(std::string name, int64_t ino, blockfs::FileType type) {
	...
	while(offset < fileSize()) {
		...
		auto contracted =
			(sizeof(DiskDirEntry) + previous_entry->nameLength + 3) & ~size_t(3);
...
async::result<frg::expected<protocols::fs::Error>> Inode::unlink(std::string name) {
	...
	while(offset < fileSize()) {
		...
		if(disk_entry->inode && name.length() == disk_entry->nameLength
		   && !memcmp(disk_entry->name, name.data(), name.length())) {
			// The directory should start with "." and "..". As those entries are never
			// deleted, we can assume that a previous entry exists.
...
async::result<void> FileSystem::init() {
	...
	HEL_CHECK(helCreateManagedMemory(
		numBlockGroups << blockPagesShift,
		0,
		&block_bitmap_backing,
		&block_bitmap_frontal
	));
	HEL_CHECK(helCreateManagedMemory(
		numBlockGroups << blockPagesShift,
		0,
		&inode_bitmap_backing,
		&inode_bitmap_frontal
	));
	...
	HEL_CHECK(helCreateManagedMemory(
		inodesPerGroup * inodeSize * numBlockGroups,
		0,
		&inode_table_backing,
		&inode_table_frontal
	));
	...
	while(true) {
		...
		if(manage.type() == kHelManageInitialize) {
			helix::Mapping table_map {
				...
				manage.length()};
			co_await device->readSectors(
				block * sectorsPerBlock + bg_offset / 512,
				table_map.get(),
				manage.length() / 512
			);
			HEL_CHECK(helUpdateMemory(
...
FileSystem::write(Inode *inode, uint64_t offset, const void *buffer, size_t length) {
	...
	/*
		...
		helix::LockMemoryView lockMemory;
		auto &&submit =
	   helix::submitLockMemoryView(helix::BorrowedDescriptor(inode->frontalMemory), &lockMemory,
	   mapOffset, mapSize, helix::Dispatcher::global()); co_await submit.async_wait();
		HEL_CHECK(lockMemory.error());
...
async::detached FileSystem::manageFileData(std::shared_ptr<Inode> inode) {
	while(true) {
		...
		co_await submit.async_wait();
		HEL_CHECK(manage.error());
		assert(manage.offset() + manage.length()
		       <= ((inode->fileSize() + 0xFFF) & ~size_t(0xFFF)));
		...
		if(manage.type() == kHelManageInitialize) {
			...
			size_t backed_size =
				std::min(manage.length(), inode->fileSize() - manage.offset());
			size_t num_blocks =
				(backed_size + (inode->fs.blockSize - 1)) / inode->fs.blockSize;

			assert(num_blocks * inode->fs.blockSize <= manage.length());
			co_await inode->fs.readDataBlocks(
				inode,
				manage.offset() / inode->fs.blockSize,
				num_blocks,
				file_map.get()
			);
	...
	for(uint32_t bg_idx = 0; bg_idx < numBlockGroups; bg_idx++) {
		...
		for(unsigned int i = 0; i < (blocksPerGroup + 31) / 32; i++) {
			...
			for(int j = 0; j < 32; j++) {
				...
				// TODO: Make sure we never return blocks higher than the max. block
				// in the SB.
	...
	for(uint32_t bg_idx = 0; bg_idx < numBlockGroups; bg_idx++) {
		...
		for(unsigned int i = 0; i < (inodesPerGroup + 31) / 32; i++) {
			...
			for(int j = 0; j < 32; j++) {
				...
				// TODO: Make sure we never return inodes higher than the max. inode
				// in the SB.
...
async::result<void> FileSystem::readDataBlocks(
	...
	while(progress < num_blocks) {
		// Block number and block count of the readSectors() command that we will issue
		// here.
		std::pair<size_t, size_t> issue;

		auto index = offset + progress;
		//		std::cout << "Reading " << index << "-th block from inode " <<
		//inode->number
		//				<< " (" << progress << "/" << num_blocks << " in request)"
		//<< std::endl;
		...
		if(index >= d_range) {
			...
			issue =
				fuse(num_blocks - progress,
				     disk_inode->data.blocks.direct + index,
				     12 - index);
		}

		//		std::cout << "Issuing read of " << issue.second
		//				<< " blocks, starting at " << issue.first <<
		//std::endl;
		...
		if(issue.first) {
			...
			memset((uint8_t *) buffer + progress * blockSize,
			       0,
			       issue.second * blockSize);
		}
...
async::result<void> FileSystem::writeDataBlocks(
	...
	size_t progress = 0;
	while(progress < num_blocks) {
		// Block number and block count of the writeSectors() command that we will issue
		// here.
		std::pair<size_t, size_t> issue;

		auto index = offset + progress;
		//		std::cout << "Write " << index << "-th block to inode " <<
		//inode->number
		//				<< " (" << progress << "/" << num_blocks << " in request)"
		//<< std::endl;
...
async::result<std::optional<std::string>> OpenFile::readEntries() {
	...
	while(offset < inode->fileSize()) {
		...
		if(disk_entry->inode) {
			...
			//			<< std::string(disk_entry->name, disk_entry->nameLength)
			//<< std::endl;
