...
extern inline __attribute__((always_inline)) HelError helLog(const char *string, size_t length) {
...
helTransferDescriptor(HelHandle handle, HelHandle universe_handle, HelHandle *out_handle) {
	...
	HelError error = helSyscall2_1(
		kHelCallTransferDescriptor,
		(HelWord) handle,
		(HelWord) universe_handle,
		&hel_out_handle
	);
...
extern inline __attribute__((always_inline)) HelError helAllocateMemory(
	...
	HelError error = helSyscall3_1(
		kHelCallAllocateMemory,
		(HelWord) size,
		(HelWord) flags,
		(HelWord) restrictions,
		&hel_handle
	);
	*handle = (HelHandle) hel_handle;
	return error;
...
helCreateIndirectMemory(size_t numSlots, HelHandle *handle) {
	HelWord helHandle;
	HelError error =
		helSyscall1_1(kHelCallCreateIndirectMemory, (HelWord) numSlots, &helHandle);
...
helCreateVirtualizedCpu(HelHandle handle, HelHandle *out_handle) {
	HelWord handle_word;
	HelError error =
		helSyscall1_1(kHelCallCreateVirtualizedCpu, (HelWord) handle, &handle_word);
	*out_handle = (HelHandle) handle_word;
	return error;
}
...
helRunVirtualizedCpu(HelHandle handle, struct HelVmexitReason *exitInfo) {
	HelError error =
		helSyscall2(kHelCallRunVirtualizedCpu, (HelWord) handle, (HelWord) exitInfo);
	return error;
}
...
extern inline __attribute__((always_inline)) HelError
helSetAffinity(HelHandle thread, uint8_t *mask, size_t size) {
	return helSyscall3(kHelCallSetAffinity, (HelWord) thread, (HelWord) mask, (HelWord) size);
};
