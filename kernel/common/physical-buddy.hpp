...
struct BuddyAccessor {
	...
	constexpr inline static AddressType illegalAddress = static_cast<AddressType>(-1);
...
private:
	...
	int traverseForSanityCheck(int8_t *slice, int order, size_t base) {
		...
		if(slice[base] == -1) {
			...
			// - marked as free (if this entry is allocated and we never descend
			// further),
			...
			// Either:
			// - all descedants are completely free (and then this entry is also
			// completely free),
...
public:
	...
	AddressType allocate(int order, int addressBits) {
		...
		if(addressBits < static_cast<int>(sizeof(AddressType) * 8)) {
			...
			// starts at _baseAddress but is not necessarily fully contained in the
			// buddy.
			AddressType addressableRange =
				(AddressType {1} << addressBits) - _baseAddress;
			if(addressableRange < (AddressType {1} << (order + _sizeShift))) {
				return illegalAddress;
			}

			// Descend until some constant number (i.e., 4) of roots are fully
			// addressable, i.e., are addressable with addressBits-many bits.
		...
		slice[allocIndex] = -1;
		if constexpr(enableBuddySanityChecking) {
			assert(slice + allocIndex
			       < buddyPointer_ + determineSize(numRoots_, tableOrder_));
		}
