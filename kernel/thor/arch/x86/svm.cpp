...
bool init() {
	...
	common::x86::wrmsr(
		0xC0000080,
		common::x86::rdmsr(0xC0000080) | (1 << 12)
...
Vcpu::Vcpu(smarter::shared_ptr<NptSpace> npt) : space {npt} {
	...
	assert(reinterpret_cast<VirtualAddr>(host_fpu_state) != static_cast<VirtualAddr>(-1)
	       && "OOM");
	memset((void *) host_fpu_state, 0, simd_state_size);

	guest_fpu_state = (uint8_t *) kernelAlloc->allocate(simd_state_size);
	assert(reinterpret_cast<VirtualAddr>(guest_fpu_state) != static_cast<VirtualAddr>(-1)
	       && "OOM");
...
HelVmexitReason Vcpu::run() {
	...
	while(true) {
		...
		asm volatile("vmsave %%rax"
			     ...
			     : "memory");  // Use vmsave / vmload to save additional state that
					   // would otherwise have to be wrmsr'ed
