...
constexpr inline arch::bit_register<uint64_t> genCapsAndId(0);
constexpr inline arch::bit_register<uint64_t> genConfig(16);
constexpr inline arch::scalar_register<uint64_t> mainCounter(240);
constexpr inline arch::bit_register<uint64_t> timerConfig0(256);
constexpr inline arch::scalar_register<uint64_t> timerComparator0(264);

// genCapsAndId register.
constexpr inline arch::field<uint64_t, bool> has64BitCounter(13, 1);
constexpr inline arch::field<uint64_t, bool> supportsLegacyIrqs(15, 1);
constexpr inline arch::field<uint64_t, uint32_t> counterPeriod(32, 32);

// genConfig register
constexpr inline arch::field<uint64_t, bool> enableCounter(0, 1);
constexpr inline arch::field<uint64_t, bool> enableLegacyIrqs(1, 1);

// timerConfig registers
namespace timer_bits {
constexpr inline arch::field<uint64_t, bool> enableInt(2, 1);
constexpr inline arch::field<uint64_t, bool> has64BitComparator(5, 1);
constexpr inline arch::field<uint64_t, bool> forceTo32Bit(8, 1);
constexpr inline arch::field<uint64_t, unsigned int> activeIrq(9, 5);
constexpr inline arch::field<uint64_t, bool> fsbEnabled(14, 1);
constexpr inline arch::field<uint64_t, bool> fsbCapable(15, 1);
constexpr inline arch::field<uint64_t, unsigned int> possibleIrqs(32, 32);
};  // namespace timer_bits
...
constexpr inline arch::scalar_register<uint8_t> channel0(64);
constexpr inline arch::bit_register<uint8_t> command(67);

constexpr inline arch::field<uint8_t, int> operatingMode(1, 3);
constexpr inline arch::field<uint8_t, int> accessMode(4, 2);

struct HpetDevice final
: IrqSink
, ClockSource
, AlarmTracker {
...
public:
	void arm(uint64_t nanos) override {
		...
		if(nanos < now) {
			...
			ticks = hpetBase.load(mainCounter)
			      + (nanos - now) / (hpetPeriod / kFemtosPerNano);
		}
		if(!_comparatorIs64Bit) {
			// TODO: We could certainly do something better here.
			//       - If the tick happens during the next main counter cycle (despite
			//       overflow),
