...
constexpr inline arch::scalar_register<uint64_t> msixMessageAddress {0};
constexpr inline arch::scalar_register<uint32_t> msixMessageData {8};
constexpr inline arch::scalar_register<uint32_t> msixVectorControl {12};
...
struct PciBusResource {
	constexpr static uint32_t io = 1;
	constexpr static uint32_t memory = 2;
	constexpr static uint32_t prefMemory = 3;
...
struct PciBus {
	PciBus(PciBridge *associatedBridge_,
...
struct PciBridge final : PciEntity {
	...
	PciBar *getBars() override { return bars; }
