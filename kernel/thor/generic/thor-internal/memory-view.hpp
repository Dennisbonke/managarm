...
struct CachePage {
	...
	constexpr static uint32_t reclaimRegistered = 0x01;
	// Page is currently being evicted (not in LRU list, but in bundle list).
	constexpr static uint32_t reclaimPosted = 0x02;
	// Page has been evicted (neither in the LRU, nor in the bundle list).
	constexpr static uint32_t reclaimInflight = 0x04;
...
constexpr inline FetchFlags fetchDisallowBacking = 1;
...
public:
	...
	virtual coroutine<frg::expected<Error>> touchRange(
		uintptr_t offset,
		size_t size,
		FetchFlags flags,
		smarter::shared_ptr<WorkQueue> wq
	...
	struct [[nodiscard]] ResizeSender {
		template<typename R>
		friend ResizeOperation<R> connect(ResizeSender sender, R receiver) {
	...
	struct [[nodiscard]] ForkSender {
		...
		template<typename R>
		friend ForkOperation<R> connect(ForkSender sender, R receiver) {
			return {sender, std::move(receiver)};
		}
		...
		[](Node &nd) {
			return async::sequence(
				...
				async::repeat_while(
					...
					[&nd] {
						auto destFetchOffset = (nd.destOffset + nd.progress)
								     & ~(kPageSize - 1);
						auto srcFetchOffset = (nd.srcOffset + nd.progress)
								    & ~(kPageSize - 1);
						return async::sequence(
							async::transform(
								nd.destView->fetchRange(
									destFetchOffset,
									0,
									nd.wq
								),
							...
							nd.wq->schedule(),
							async::invocable([&nd] {
								auto destMisalign = (nd.destOffset
										     + nd.progress)
										  % kPageSize;
								auto srcMisalign =
									(nd.srcOffset + nd.progress)
									% kPageSize;
								...
								PageAccessor destAccessor {
									destPhysical};
								PageAccessor srcAccessor {
									srcPhysical};
								memcpy((uint8_t *) destAccessor.get(
								       ) + destMisalign,
								       (uint8_t *) srcAccessor.get()
									       + srcMisalign,
								       chunk);
...
, GlobalFutexSpace {
	...
	T *accessImmediate(uintptr_t offset) {
		static_assert(
			sizeof(T) <= kPageSize
		);  // Otherwise, the assert below will always fail.
...
struct CopyOnWriteMemory final
: MemoryView
, GlobalFutexSpace /*, MemoryObserver */ {
public:
	CopyOnWriteMemory(
		smarter::shared_ptr<MemoryView> view,
		uintptr_t offset,
...
struct GlobalFutex {
	...
	GlobalFutex(
		smarter::shared_ptr<GlobalFutexSpace> space,
		uintptr_t offset,
