...
coroutine<void> runStdioRequests(LaneHandle lane) {
	...
	while(true) {
		...
		if(req.req_type() == managarm::fs::CntReqType::WRITE) {
			...
			infoLogger() << "\e[31m"
					"thor: Illegal request type "
				     << (int32_t) req.req_type()
				     << " for kernel provided stdio file"
					"\e[39m"
				     << frg::endlog;
	...
	while(true) {
		...
		if(req.req_type() == managarm::fs::CntReqType::READ) {
			...
			infoLogger() << "\e[31m"
					"thor: Illegal request type "
				     << (int32_t) req.req_type()
				     << " for kernel provided regular file"
					"\e[39m"
				     << frg::endlog;
	...
	while(true) {
		...
		if(req.req_type() == managarm::fs::CntReqType::PT_READ_ENTRIES) {
			if(file->index < file->node->numEntries()) {
				...
				frg::unique_memory<KernelAlloc> respBuffer {
					*kernelAlloc,
					ser.size()};
				memcpy(respBuffer.data(), ser.data(), ser.size());
				auto respError = co_await SendBufferSender {
					conversation,
					std::move(respBuffer)};
				// TODO: improve error handling here.
				assert(respError == Error::success);
			...
			infoLogger() << "\e[31m"
					"thor: Illegal request type "
				     << (int32_t) req.req_type()
				     << " for kernel provided directory file"
					"\e[39m"
				     << frg::endlog;
	...
	while(true) {
		...
		if(interrupt == kIntrPanic) {
			...
			*_thread->_executor.result1() = mapResult.value();
			if(auto e = Thread::resumeOther(remove_tag_cast(_thread));
			   e != Error::success) {
