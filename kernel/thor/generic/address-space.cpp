...
[[maybe_unused]] void logRss(VirtualSpace *space) {
	if(!logUsage) {
		return;
	}
	auto rss = space->rss();
	...
	infoLogger() << "thor:     Physical usage: " << (physicalAllocator->numUsedPages() * 4)
		     << " KiB, kernel usage: " << (kernelMemoryUsage / 1024) << " KiB"
		     << frg::endlog;
}
}  // namespace
...
bool HoleAggregator::check_invariant(HoleTree &tree, Hole *hole) {
	...
	if(hole->largestHole != size) {
		infoLogger() << "largestHole violation: "
			     << "Expected " << size << ", got " << hole->largestHole << "."
			     << frg::endlog;
		return false;
	}
...
coroutine<frg::expected<Error, VirtualAddr>> VirtualSpace::map(
	...
	smarter::shared_ptr<Mapping> mapping;
	{
		...
		} else if((flags & mask) == (kMapProtRead | kMapProtWrite) || (flags & mask) == kMapProtWrite) {
			// W is upgraded to RW.
			mappingFlags |= MappingFlags::protRead | MappingFlags::protWrite;
	...
	while(true) {
		...
		if(!remapOutcome) {
			if(remapOutcome.error() == Error::spuriousOperation) {
				...
				// They should be rare. If they happen too often, something is
				// probably wrong!
				infoLogger() << "\e[33m"
						"thor: Spurious page fault"
						"\e[39m"
					     << frg::endlog;
			} else {
				assert(remapOutcome.error() == Error::fault);
				infoLogger() << "\e[33m"
						"thor: Page still not available after"
						" fetchRange()"
						"\e[39m"
					     << frg::endlog;
				continue;
...
coroutine<size_t> VirtualSpace::readPartialSpace(
	...
	while(progress < size) {
		...
		while(progress < size) {
			...
			// TODO: there is no real reason why we need to page aligned here; however,
			// the
