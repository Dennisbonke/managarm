	...
	for(auto mapping : *(proc->vmContext())) {
		...
		if(mapping.backingFile().get()) {
			...
			while(true) {
				...
				// If we are at the origin of a mount point, traverse that mount
				// point.
				ViewPath traversed;
				if(vp.second == vp.first->getOrigin()) {
					...
					assert(anchor
					);  // Non-root mounts must have anchors in their parents.
...
observeThread(std::shared_ptr<Process> self, std::shared_ptr<Generation> generation) {
	...
	while(true) {
		...
		if(observe.observation() == kHelObserveSuperCall + posix::superAnonAllocate) {
			...
			// Setup post supercall registers in both threads and finally resume the
			// threads.
			gprs[kHelRegError] = kHelErrNone;
			...
			// TODO: This should only happen is raising succeeds. Move it somewhere
			// else?
			gprs[kHelRegError] = 0;
			...
			// If the process signalled itself, we should process the signal before
			// resuming.
			bool killed = false;
			if(self->checkOrRequestSignalRaise()) {
				auto active = co_await self->signalContext()->fetchSignal(
