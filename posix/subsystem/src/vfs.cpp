...
async::result<void> populateRootView() {
	...
	while(!stack.empty()) {
		...
		while(true) {
			...
			// std::cout << "posix: Importing " << item.second + "/" + resp.path() <<
			// std::endl;
		...
		if(name == ".") {
			...
			if(_currentPath == _rootPath) {
				...
				if(auto parent = _currentPath.first->getParent(); parent) {
					...
					// TODO: We have to decide if the following is something we
					// want to support:
			...
			if(_currentPath.second->getTarget()->hasTraverseLinks()) {
				...
				if(!result) {
					...
					if(result.error() == Error::illegalOperationTarget) {
						std::cout
							<< "\e[33mposix: Illegal operation target in PathResolver::resolve\e[39m"
							<< std::endl;
