...
loadElfImage(SharedFilePtr file, VmContext *vmContext, uintptr_t base) {
	...
	for(int i = 0; i < ehdr.e_phnum; i++) {
		...
		if(phdr->p_type == PT_LOAD) {
			...
			if(!(phdr->p_flags & PF_W)) {
				...
				if((phdr->p_offset & (kPageSize - 1))) {
					// TODO: We could handle the case where p_offset and p_vaddr
					// are
					//       "equally misaligned".
				...
				if((phdr->p_flags & (PF_R | PF_W | PF_X)) == (PF_R | PF_X)) {
					...
					std::cout
						<< "posix: Illegal combination of segment permissions"
						<< std::endl;
					co_return Error::badExecutable;
				}
			} else {
		...
		} else if(phdr->p_type == PT_DYNAMIC || phdr->p_type == PT_INTERP || phdr->p_type == PT_TLS || phdr->p_type == PT_GNU_EH_FRAME || phdr->p_type == PT_GNU_STACK || phdr->p_type == PT_GNU_RELRO) {
