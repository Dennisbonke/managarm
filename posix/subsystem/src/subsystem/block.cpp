...
, drvcore::BlockDevice {
	...
	void composeUevent(drvcore::UeventProperties &ue) override { ue.set("SUBSYSTEM", "block"); }
...
async::detached run() {
	...
	handler = mbus::ObserverHandler {}.withAttach(
		[](mbus::Entity entity, mbus::Properties properties) -> async::detached {
			auto diskId = std::stoll(
				std::get<mbus::StringItem>(properties.at("unix.diskid")).value
			);
			auto diskName = diskNames.at(diskId);

			auto name = std::string("sd") + diskName
				  + std::get<mbus::StringItem>(properties.at("unix.partid")).value;
			...
			// We use 8 here, the major for SCSI devices and allocate minors
			// sequentially. Note that this is not really correct as the minor of a
			// partition depends on the minor of the whole device (see Linux devices.txt
			// documentation).
