...
constexpr inline ResolveFlags resolvePrefix = (1 << 4);
// The path must not refer to a directory (not trailing slash allowed).
constexpr inline ResolveFlags resolveNoTrailingSlash = (1 << 2);
constexpr inline ResolveFlags resolveDontFollow = (1 << 1);
...
private:
	struct Compare {
		...
		bool operator()(
			const std::shared_ptr<MountView> &a,
			const std::shared_ptr<FsLink> &b
		) const {
			return a->getAnchor() < b;
		}

		bool operator()(
			const std::shared_ptr<FsLink> &a,
			const std::shared_ptr<MountView> &b
		) const {
...
resolve(ViewPath root, ViewPath workdir, std::string name, Process *process, ResolveFlags flags = 0
);
