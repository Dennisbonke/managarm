...
std::shared_ptr<Link> DirectoryNode::createRootDirectory() {
	...
	auto self_thread_link = std::make_shared<Link>(
		the_node->shared_from_this(),
		"thread-self",
		std::make_shared<SelfThreadLink>()
	);
...
async::result<frg::expected<Error, std::shared_ptr<FsLink>>>
DirectoryNode::link(std::string name, std::shared_ptr<FsNode> target) {
	co_return Error::noSuchFile;
}
...
expected<std::string> SelfThreadLink::readSymlink(FsLink *link, Process *process) {
	co_return "/proc/" + std::to_string(process->pid()) + "/task/"
		+ std::to_string(process->tid());
}
...
async::result<std::string> StatusNode::show() {
	...
	stream << "Umask: 0022\n";  // Hardcoded to 0022, which is what we hardcode in the mlibc
				    // sysdeps.
	...
	stream << "FDSize: 256\n";  // Pick a sane default, I don't believe we have a real maximum
				    // here.
