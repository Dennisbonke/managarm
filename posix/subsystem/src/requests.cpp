...
serveRequests(std::shared_ptr<Process> self, std::shared_ptr<Generation> generation) {
	...
	while(true) {
		...
		if(preamble.id() == bragi::message_id<managarm::posix::GetPidRequest>) {
			auto req =
				bragi::parse_head_only<managarm::posix::GetPidRequest>(recv_head);
			if(!req) {
				std::cout << "posix: Rejecting request due to decoding failure"
					  << std::endl;
				break;
			...
			if(req.flags() & WUNTRACED) {
				std::cout
					<< "\e[31mposix: WAIT flag WUNTRACED is silently ignored\e[39m"
					<< std::endl;
			}

			if(req.flags() & WCONTINUED) {
				std::cout
					<< "\e[31mposix: WAIT flag WCONTINUED is silently ignored\e[39m"
					<< std::endl;
			}
			...
			if(req.mode() == RUSAGE_SELF) {
				...
				std::cout
					<< "\e[31mposix: GET_RESOURCE_USAGE mode is not supported\e[39m"
					<< std::endl;
				...
				if(!sourceResult) {
					if(sourceResult.error()
					   == protocols::fs::Error::fileNotFound) {
						...
						std::cout
							<< "posix: Unexpected failure from resolve()"
							<< std::endl;
						co_return;
					}
				}
			...
			auto req = bragi::parse_head_tail<managarm::posix::AccessAtRequest>(
				recv_head,
				tail
			);
			...
			if(req->flags()) {
				if(req->flags() & AT_SYMLINK_NOFOLLOW) {
					...
					std::cout
						<< "posix: ACCESSAT flag handling AT_EACCESS is unimplemented"
						<< std::endl;
				} else {
					std::cout
			...
			auto req = bragi::parse_head_tail<managarm::posix::MkdirAtRequest>(
				recv_head,
				tail
			);
			...
			// This catches cases where associatedLink is called on a file, but the file
			// doesn't implement that. Instead of blowing up, return ENOENT.
			...
			if(!req->path().size()) {
				...
				if(req->flags() & AT_SYMLINK_NOFOLLOW) {
					std::cout
						<< "posix: AT_SYMLINK_FOLLOW is unimplemented for utimensat"
						<< std::endl;
				}
				...
				if(req->flags() & managarm::posix::OpenFlags::OF_PATH) {
					...
					if(!fileResult) {
						if(fileResult.error() == Error::noBackingDevice) {
							...
							std::cout
								<< "posix: Unexpected failure from open()"
								<< std::endl;
							co_return;
						}
					}
			...
			if(closeErr != Error::success) {
				if(closeErr == Error::noSuchFile) {
					...
					std::cout
						<< "posix: Unhandled error returned from closeFile"
						<< std::endl;
					break;
				}
			}
			...
			if(req->flags()) {
				if(req->flags() & AT_REMOVEDIR) {
					std::cout
						<< "posix: UNLINKAT flag AT_REMOVEDIR handling unimplemented"
						<< std::endl;
				} else {
					std::cout
						<< "posix: UNLINKAT flag handling unimplemented with unknown flag: "
						<< req->flags() << std::endl;
					co_await sendErrorResponse(
						managarm::posix::Errors::ILLEGAL_ARGUMENTS
					);
			...
			std::cout
				<< "\e[31mposix: RMDIR: always removes the directory, even when not empty\e[39m"
				<< std::endl;
			std::shared_ptr<FsLink> target_link;
			...
			if(req->domain() == AF_UNIX) {
				...
				std::cout
					<< "posix: SOCKET: Handle unknown protocols families, this is: "
					<< req->domain() << std::endl;
				co_await sendErrorResponse(
					managarm::posix::Errors::ILLEGAL_ARGUMENTS
				);
			...
			if(req->domain() != AF_UNIX) {
				std::cout << "\e[31mposix: socketpair() with domain "
					  << req->domain() << " is not implemented correctly\e[39m"
					  << std::endl;
				co_await sendErrorResponse(
					managarm::posix::Errors::ADDRESS_FAMILY_NOT_SUPPORTED
				);
				continue;
			}
			if(req->socktype() != SOCK_DGRAM && req->socktype() != SOCK_STREAM
			   && req->socktype() != SOCK_SEQPACKET) {
				std::cout << "\e[31mposix: socketpair() with socktype "
					  << req->socktype()
			...
			if(req->protocol() && req->protocol() != PF_UNSPEC) {
				std::cout << "\e[31mposix: socketpair() with protocol "
					  << req->protocol()
					  << " is not implemented correctly\e[39m" << std::endl;
				co_await sendErrorResponse(
					managarm::posix::Errors::PROTOCOL_NOT_SUPPORTED
				);
				continue;
			}
			...
			if(type == VfsType::charDevice || type == VfsType::blockDevice) {
				...
				if(!result) {
					if(result.error() == Error::illegalOperationTarget) {
						...
						std::cout
							<< "posix: Unexpected failure from mksocket()"
							<< std::endl;
						co_return;
					}
				}
			...
			if(group) {
				...
				// Not found, making it if pgid and pid match, or if pgid is 0,
