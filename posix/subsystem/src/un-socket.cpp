...
constexpr static bool logSockets = false;
...
public:
	...
	readSome(Process *, void *data, size_t max_length) override {
		...
		if(logSockets) {
			std::cout << "posix: Read from socket \e[1;34m" << structName() << "\e[0m"
				  << std::endl;
		}
	...
	writeAll(Process *process, const void *data, size_t length) override {
		...
		if(logSockets) {
			std::cout << "posix: Write to socket \e[1;34m" << structName() << "\e[0m"
				  << std::endl;
		}
	...
	async::result<frg::expected<Error, AcceptResult>> accept(Process *process) override {
		if(_acceptQueue.empty() && nonBlock_) {
			if(logSockets) {
				std::cout << "posix: UNIX socket would block on accept"
					  << std::endl;
			}
			co_return Error::wouldBlock;
		}
	...
	pollWait(Process *, uint64_t past_seq, int mask, async::cancellation_token cancellation)
		...
		//		std::cout << "posix: pollWait(" << past_seq << ") on \e[1;34m" <<
		//structName() << "\e[0m"
		//				<< " returns (" << _currentSeq
		//				<< ", " << edges << ")" << std::endl;
	...
	bind(Process *process, const void *addr_ptr, size_t addr_length) override {
		...
		if(_nameType == NameType::abstract) {
			...
			auto res = globalBindMap.insert(
				{std::weak_ptr<FsNode> {node->getTarget()}, this}
			);
			if(!res.second) {
				co_return protocols::fs::Error::addressInUse;
			}
		...
		if(id == managarm::fs::GenericIoctlRequest::message_id) {
			...
			case FIONREAD: {
				...
				if(_currentState != State::connected) {
					...
					auto packet = &_recvQueue.front();
					resp.set_fionread_count(
						packet->buffer.size() - packet->offset
					);
				}
				break;
			}
