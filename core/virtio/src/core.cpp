...
Queue *LegacyPciTransport::setupQueue(unsigned int queue_index) {
	...
	auto available =
		reinterpret_cast<spec::AvailableRing *>((char *) window + available_offset);
	auto used = reinterpret_cast<spec::UsedRing *>((char *) window + used_offset);
	_queues[queue_index] = std::make_unique<LegacyPciQueue>(
		this,
		queue_index,
		queue_size,
		table,
...
void StandardPciTransport::finalizeFeatures() {
	...
	_commonSpace().store(
		PCI_DEVICE_STATUS,
		_commonSpace().load(PCI_DEVICE_STATUS) | FEATURES_OK
	);
...
discover(protocols::hw::Device hw_device, DiscoverMode mode) {
	...
	if(mode == DiscoverMode::legacyOnly || mode == DiscoverMode::transitional) {
		...
		if(info.barInfo[0].ioType == protocols::hw::IoType::kIoTypePort) {
			...
			// Reset the device.
			arch::io_space legacy_space {
				static_cast<uint16_t>(info.barInfo[0].address)};
...
scatterGather(HostToDeviceType, Chain &chain, Queue *queue, arch::dma_buffer_view view) {
	...
	while(offset < view.size()) {
		...
		auto chunk =
			std::min(view.size() - offset, page_size - (address & (page_size - 1)));
...
, _progressHead {0} {
	...
	_availableExtra =
		new(spec::AvailableExtra::get(available, _queueSize)) spec::AvailableExtra;
